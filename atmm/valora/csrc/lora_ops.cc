...
inline bool launch_bgmv_kernel(T* Y, const T* X, const T* W,
                              ...
                              //  const int64_t* indicies,
                               uint64_t qkvo,
                               uint16_t in_features, uint16_t out_features, int64_t batch_size,
                               const T* lora_scales,
...
#define CASE_ONESIDE(_T, feat_in, feat_out, thd_x_value , thd_y_value, thd_z_value,wrap_x_value,wrap_y_value, wrap_z_value)                           \
  case hash_vl(feat_in, feat_out, thd_x_value , thd_y_value, thd_z_value,wrap_x_value,wrap_y_value, wrap_z_value):                                   \
    bgmv_kernel<feat_in, feat_out, thd_x_value , thd_y_value, thd_z_value,wrap_x_value,wrap_y_value, wrap_z_value>(Y, X, W, start_indicies, lora_ranks, loc_indicies, \
...
void dispatch_bgmv(torch::Tensor y, torch::Tensor x, torch::Tensor w, 
                   torch::Tensor start_indicies, torch::Tensor lora_ranks, torch::Tensor loc_indicies, 
                  //  torch::Tensor indicies,
                   int64_t qkvo, 
                   torch::Tensor lora_scales, 
                   torch::Tensor output_counts, 
                   torch::Tensor rank_counts,
                   torch::Tensor lora_ids,
                   torch::Tensor start_ids, 
                   torch::Tensor tmp_d, 
                   int64_t num_problems, 
                  int32_t thrd_x,
                  int32_t thrd_y,
